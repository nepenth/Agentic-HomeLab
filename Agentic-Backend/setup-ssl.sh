    ...
    3)
        ...
        if [[ $prereq == "y" || $prereq == "Y" ]]; then
            ...
            if [ $? -eq 0 ]; then
                ...
                sudo cp /etc/letsencrypt/live/ollama.example.invalid/fullchain.pem nginx/ssl/
                sudo cp /etc/letsencrypt/live/ollama.example.invalid/privkey.pem nginx/ssl/
...
if [[ -f "nginx/ssl/fullchain.pem" && -f "nginx/ssl/privkey.pem" ]]; then
    ...
    echo "  - https://ollama.example.invalid/api/v1/"
    echo "  - https://ollama.example.invalid/docs (if debug is enabled)"
    echo "  - https://ollama.example.invalid/flower/ (Celery monitoring)"
else
