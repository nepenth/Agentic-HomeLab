...
class SyncEmailSyncService:
        ...
        try:
            ...
            for email_id in email_ids:
                try:
                    ...
                    # Fetch email (use BODY.PEEK[] to avoid marking as read)
                    # RFC822 automatically sets \Seen flag, BODY.PEEK[] is read-only
                    status, msg_data = connection.fetch(email_id, '(BODY.PEEK[])')
                    if status != 'OK':
    ...
    def cleanup_old_deleted_emails(self, db: Session, days_threshold: int = 60) -> Dict[str, int]:
        """
        Permanently delete emails that were marked as deleted more than X days ago.

        This implements the 60-day soft-delete retention policy:
        1. Find all emails where deleted_at is not null AND older than threshold
        2. Delete associated embeddings and attachments (CASCADE will handle this)
        3. Permanently delete the email records
        ...
        Returns:
            Dict with counts of emails and attachments deleted
        """
        from app.db.models.email import EmailAttachment

        self.logger.info(f"Starting cleanup of emails deleted more than {days_threshold} days ago")
        ...
        emails_count = len(emails_to_delete)
        attachments_count = 0

        if emails_count == 0:
            self.logger.info("No emails found for cleanup")
            return {"emails_deleted": 0, "attachments_deleted": 0}

        # Count attachments that will be cascade deleted
        email_ids = [email.id for email in emails_to_delete]
        attachments_count = db.query(EmailAttachment).filter(
            EmailAttachment.email_id.in_(email_ids)
        ).count()

        # Delete the emails (CASCADE will handle embeddings and attachments)
        ...
        self.logger.info(
            f"Cleanup completed: {emails_count} emails and {attachments_count} attachments "
            f"permanently deleted (deleted_at < {cutoff_date.isoformat()})"
        )
