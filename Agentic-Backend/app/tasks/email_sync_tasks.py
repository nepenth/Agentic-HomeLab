...
def _periodic_sync_scheduler_sync():
    """Synchronous implementation of periodic sync scheduler."""
...
def sync_single_account(self, account_id: str, sync_type: str = "incremental", force_sync: bool = False):
    ...
    """
    try:
        return _sync_single_account_sync(account_id, sync_type, force_sync)
    except Exception as exc:
...
def _sync_single_account_sync(account_id: str, sync_type: str, force_sync: bool):
    """
    Pure synchronous wrapper for single account sync.

    Uses sync database session and sync email service to avoid
    all async/event loop complications in Celery workers.
    """
    logger.info(f"Starting sync for account {account_id} (type: {sync_type})")

    try:
        # Import sync service and database session
        ...
        # Use synchronous database session - NO ASYNC!
        ...
        logger.info(
            f"Completed sync for account {account_id}: "
