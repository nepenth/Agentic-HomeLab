...
# Test from container
docker-compose exec api curl http://ollama.example.invalid:11434/api/tags

# Check if model exists
curl http://ollama.example.invalid:11434/api/tags | jq '.models[].name'
```
...
curl -X POST http://ollama.example.invalid:11434/api/pull \
...
telnet ollama.example.invalid 11434

# Add to hosts file if DNS issues
echo "IP_ADDRESS ollama.example.invalid" >> /etc/hosts
```
...
echo -e "\nOllama connectivity:"
curl -s http://ollama.example.invalid:11434/api/tags > /dev/null && echo "✅ OK" || echo "❌ FAILED"
```
